...
namespace neatCpp {
    ...
    void Genome::generateNetwork() {
        ...
        // std::cout << "pushingBack" << std::endl;
        // add the connection to the nodes
        const long int connectionsLen = connections.size();
        for (long int i = 0; i < connectionsLen;i++) {
            // std::cout << "pushingBack" << i << std::endl;
    ...
    void Genome::addNode() {
        ...
        Node* oriFromNode = pickedConnection->getFromNode();
        // delete connection

        connections.erase(connections.begin() + connectionIndex);

        // create new node
        Node* newNode = new Node(nextNode, oriFromNode->getLayer() + 1, false);
        for (long int i = 0; i < nodesLen;++i) {
            if (nodes[i]->getLayer() > oriFromNode->getLayer()) {
                nodes[i]->increaseLayer(1);
            }
        }

        // new connections
        Connection* newConnection1 = new Connection(oriFromNode, newNode, 1);
        Connection* newConnection2 = new Connection(newNode, pickedConnection->getToNode(), pickedConnection->getWeight());
        // remove connection from oriNode
        oriFromNode->removeConnection(pickedConnection);
        //
        ++layers;
        ...
        // pickedConnection->setEnable(false);
    }
    ...
    int Genome::getInputSize() const { return inputs; }
    int Genome::getOutputSize() const { return outputs; }
